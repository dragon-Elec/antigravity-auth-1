...
const MODELS: ModelTest[] = [
  ...
  {
    model: 'google/antigravity-gemini-3.7-flash-low',
    category: 'antigravity-gemini',
  },
  {
    model: 'google/antigravity-gemini-3.7-flash-medium',
    category: 'antigravity-gemini',
  },
  {
  ...
  // Antigravity Claude
  {
    ...
    category: 'antigravity-claude',
  },
  {
    model: 'google/antigravity-claude-opus-4-6-thinking-low',
    category: 'antigravity-claude',
  },
  {
    model: 'google/antigravity-claude-opus-4-6-thinking-max',
    category: 'antigravity-claude',
  },
...
function parseArgs(): {
  ...
  return {
    ...
    timeout:
      timeoutIdx !== -1
        ? parseInt(args[timeoutIdx + 1] || '120000', 10)
        : DEFAULT_TIMEOUT_MS,
  }
}
...
async function main(): Promise<void> {
  ...
  if (filterModel)
    tests = tests.filter(
      (t) => t.model === filterModel || t.model.endsWith(filterModel),
    )
