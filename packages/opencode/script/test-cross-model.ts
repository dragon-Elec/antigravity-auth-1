...
const geminiHistoryWithThinkingAndToolCall = {
  contents: [
    ...
    },
    {
      ...
      parts: [
        {
          text: 'The root filesystem is 62% utilized, which is moderate usage.',
        },
      ],
    },
  ],
...
function runTests(): void {
  ...
  if (geminiFamily === 'gemini' && claudeFamily === 'claude') {
    ...
    console.log(
      `  ❌ FAIL: Expected gemini/claude, got ${geminiFamily}/${claudeFamily}`,
    )
    failed++
  }
  ...
  console.log(
    '  Input: Gemini session with thinking + tool call containing thoughtSignature',
  )

  const result = sanitizeCrossModelPayload(
    geminiHistoryWithThinkingAndToolCall,
    {
  ...
  if (thinkingPart.thoughtSignature === undefined) {
    console.log(
      '  ✅ PASS: Top-level thoughtSignature stripped from thinking part',
    )
    passed++
  } else {
  ...
  if (toolPart.metadata?.google?.thoughtSignature === undefined) {
    console.log(
      '  ✅ PASS: Nested metadata.google.thoughtSignature stripped from tool part',
    )
    passed++
  } else {
  ...
  if (result.modified && result.signaturesStripped === 2) {
    console.log(
      `  ✅ PASS: Sanitization metrics correct (modified=true, stripped=${result.signaturesStripped})`,
    )
    passed++
  } else {
    console.log(
      `  ❌ FAIL: Metrics incorrect (modified=${result.modified}, stripped=${result.signaturesStripped})`,
    )
    failed++
  }

  console.log('\nTest 3: Same model family - no sanitization')
  const sameFamily = sanitizeCrossModelPayload(
    geminiHistoryWithThinkingAndToolCall,
    {
