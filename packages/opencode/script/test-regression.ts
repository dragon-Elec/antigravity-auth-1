...
const HEAVY_TESTS: MultiTurnTest[] = [
  {
    ...
    turns: [
      ...
      {
        prompt: 'Now read tsconfig.json and tell me the target',
        model: CLAUDE_SONNET,
      },
      { prompt: 'Run: ls -la src/plugin | head -5', model: GEMINI_FLASH },
      {
        prompt: 'Read src/plugin/auth.ts and summarize in 1 sentence',
        model: CLAUDE_SONNET,
      },
      { prompt: 'Run: wc -l src/plugin/*.ts | tail -3', model: GEMINI_FLASH },
      {
        prompt:
          'Read README.md first 50 lines and tell me what this project does',
        model: CLAUDE_SONNET,
      },
      { prompt: 'Run: git log --oneline -3', model: GEMINI_FLASH },
      {
        prompt: 'Summarize everything we discussed in 3 bullet points',
        model: CLAUDE_SONNET,
      },
    ],
    errorPatterns: ERROR_PATTERNS,
  ...
  },
  {
    ...
    turns: [
      {
        prompt: 'Read package.json and extract the version number only',
        model: GEMINI_FLASH,
      },
      { prompt: 'Run: ls src/plugin/*.ts | head -3', model: CLAUDE_SONNET },
      { prompt: 'Read src/plugin/auth.ts first 30 lines', model: GEMINI_FLASH },
      {
        prompt: "Use grep to find 'export' in src/plugin/auth.ts",
        model: CLAUDE_SONNET,
      },
      { prompt: "Run: echo 'checkpoint 1' && date", model: GEMINI_FLASH },
      {
        prompt: 'Read tsconfig.json and tell me the module type',
        model: CLAUDE_SONNET,
      },
      { prompt: 'Use glob to find all *.test.ts files', model: GEMINI_FLASH },
      {
        prompt: 'Read src/plugin/token.ts first 20 lines',
        model: CLAUDE_SONNET,
      },
      {
        prompt: 'Run: wc -l src/plugin/*.ts | sort -n | tail -5',
        model: GEMINI_FLASH,
      },
      {
        prompt: 'What files have we read so far? List them.',
        model: CLAUDE_SONNET,
      },
      {
        prompt: 'Read src/plugin/request.ts first 25 lines',
        model: GEMINI_FLASH,
      },
      {
        prompt: "Use grep to find 'async' in src/plugin/request.ts",
        model: CLAUDE_SONNET,
      },
      { prompt: "Run: echo 'checkpoint 2' && pwd", model: GEMINI_FLASH },
      {
        prompt: 'Read src/plugin/storage.ts first 20 lines',
        model: CLAUDE_SONNET,
      },
      {
        prompt: 'Use lsp_diagnostics on src/plugin/token.ts',
        model: GEMINI_FLASH,
      },
      ...
      {
        prompt: "Run: echo 'final checkpoint' && echo 'all done'",
        model: GEMINI_FLASH,
      },
      {
        prompt: 'Summarize this entire conversation in 5 bullet points',
        model: CLAUDE_SONNET,
      },
    ],
    errorPatterns: ERROR_PATTERNS,
...
function generateEnduranceTest(turnCount: number): TurnConfig[] {
  ...
  const prompts = [
    {
      prompt: 'What is {n} + {n}? Answer with just the number.',
      model: GEMINI_FLASH,
    },
    ...
    {
      prompt: 'What turn number are we on? Just say the number.',
      model: GEMINI_FLASH,
    },
    ...
    {
      prompt: 'What have we done in last 3 turns? Brief answer.',
      model: CLAUDE_SONNET,
    },
  ]
  ...
  return new Promise((resolve) => {
    ...
    proc.on('close', (code) => {
      ...
      if (!extractedSessionId) {
        const match =
          stdout.match(/session[:\s]+([a-zA-Z0-9_-]+)/i) ||
...
async function deleteSession(sessionId: string): Promise<void> {
  return new Promise((resolve) => {
    const proc = spawn(
      'opencode',
      ['session', 'delete', sessionId, '--force'],
      {
        stdio: ['ignore', 'pipe', 'pipe'],
        timeout: 10000,
        cwd: process.cwd(),
...
async function runConcurrentTest(test: ConcurrentTest): Promise<TestResult> {
  ...
  process.stdout.write(
    `  Spawning ${test.concurrentRequests} concurrent requests...`,
  )
...
async function runMultiTurnTest(test: MultiTurnTest): Promise<TestResult> {
  ...
  for (let index = 0; index < test.turns.length; index++) {
    ...
    const model =
      typeof turn === 'string' ? test.model : (turn.model ?? test.model)
    const turnStart = Date.now()

    process.stdout.write(
      `\r  Progress: ${index + 1}/${test.turns.length} turns...`,
    )
...
async function main(): Promise<void> {
  ...
  const concurrentCount = CONCURRENT_TESTS.reduce(
    (sum, t) => sum + t.concurrentRequests,
    0,
  )
  console.log(
  ...
  for (const test of tests) {
    ...
    if (result.success) {
      console.log(
        `  Status: ✅ PASS (${result.turnsCompleted}/${test.turns.length} turns, ${(result.duration / 1000).toFixed(1)}s)`,
      )
    } else {
      console.log(`  Status: ❌ FAIL`)
      console.log(`    Error: ${result.error}`)
      console.log(
        `    Completed: ${result.turnsCompleted}/${test.turns.length} turns`,
      )
    }
  ...
  if (suite === 'heavy' || suite === 'all' || runConcurrentOnly || filterName) {
    ...
    if (concurrentTests.length > 0) {
      ...
      for (const test of concurrentTests) {
        ...
        if (result.success) {
          console.log(
            `  Status: ✅ PASS (${result.turnsCompleted} requests, ${(result.duration / 1000).toFixed(1)}s)`,
          )
        } else {
          console.log(`  Status: ❌ FAIL`)
          console.log(`    Error: ${result.error}`)
