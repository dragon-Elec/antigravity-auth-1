...
describe('updateOpencodeConfig', () => {
  ...
  test('creates new config with default structure when file does not exist', async () => {
    ...
    expect(writtenConfig.plugin).toContain(
  ...
  test('replaces existing google models with plugin models', async () => {
    ...
    expect(
      writtenConfig.provider.google.models[
  ...
  test('includes only agy-supported model definitions and whitelists them', async () => {
    ...
    expect(Object.keys(models).sort()).toEqual(
      Object.keys(OPENCODE_MODEL_DEFINITIONS).sort(),
    )
    expect(writtenConfig.provider.google.whitelist).toEqual(
      Object.keys(OPENCODE_MODEL_DEFINITIONS),
    )
  })
