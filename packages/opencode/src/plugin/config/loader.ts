...
function loadModelsFile(path: string): Record<string, any> | null {
  try {
    ...
    log.warn('Failed to load decoupled models file', {
      path,
      error: String(error),
    })
    return null
...
export function loadDecoupledModels(directory: string): Record<string, any> {
  ...
  const projectJsoncPath = join(
    directory,
    '.opencode',
    'antigravity-models.jsonc',
  )

  // User level (prefer jsonc if both exist)
  const userModels = loadModelsFile(
    existsSync(userJsoncPath) ? userJsoncPath : userJsonPath,
  )
  if (userModels) {
  ...
  const projectModels = loadModelsFile(
    existsSync(projectJsoncPath) ? projectJsoncPath : projectJsonPath,
  )
  if (projectModels) {
