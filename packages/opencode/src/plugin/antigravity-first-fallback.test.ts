...
describe('Antigravity-first fallback', () => {
  ...
  describe('Pre-check fallback logic', () => {
    ...
    it('should only fall back to gemini-cli when ALL accounts exhausted antigravity', () => {
      ...
      expect(
        manager.hasOtherAccountWithAntigravityAvailable(0, 'gemini', null),
      ).toBe(false)
      expect(
        manager.hasOtherAccountWithAntigravityAvailable(1, 'gemini', null),
      ).toBe(false)
      ...
      expect(manager.getAvailableHeaderStyle(accounts[0]!, 'gemini')).toBe(
        'gemini-cli',
      )
    })
  })
