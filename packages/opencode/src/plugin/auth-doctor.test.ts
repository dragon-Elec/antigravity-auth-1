...
describe('formatAuthDoctorReport', () => {
  ...
  it('formats runtime metadata when provided', () => {
    const report = createAuthDoctorReport({
      ...
      storage: storage(),
      runtime: {
        antigravityVersion: '1.19.0',
        antigravityVersionSource: 'api',
      },
    })

    expect(formatAuthDoctorReport(report)).toContain(
      'Antigravity version: 1.19.0 (api)',
    )
  })
})
