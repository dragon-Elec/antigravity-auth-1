...
describe('resolveModelWithTier', () => {
  ...
  describe('Gemini 3.7 Flash Antigravity routes', () => {
    it.each([
      ...
      ['antigravity-gemini-3.7-flash-low', 'gemini-3.7-flash-low', 1000, 'low'],
      [
  ...
  describe('Image models', () => {
    ...
    it('resolves the live Gemini 3.1 Flash Image route without thinking', () => {
      expect(
        resolveModelWithTier('antigravity-gemini-3.1-flash-image'),
      ).toMatchObject({
  ...
  describe('GPT-OSS', () => {
    it('resolves the only live GPT-OSS route with its captured thinking budget', () => {
      expect(
        resolveModelWithTier('antigravity-gpt-oss-120b-medium'),
      ).toMatchObject({
    ...
    it('maps the short GPT-OSS alias to the medium route', () => {
      expect(resolveModelWithTier('antigravity-gpt-oss-120b').actualModel).toBe(
        'gpt-oss-120b-medium',
      )
    })
  })
